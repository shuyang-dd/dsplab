...
class ChorusEffect(Function):
    def __init__(self, rate, block_len):
        ...
        # write index (initialize to middle of buffer)
        self.kw = int(0.5 * self.buffer_len)
    ...
    def apply(self, view, input_tuple):
        ...
        for n in range(0, self.block_len):
            ...
            diff_block[n] = int(
                self.G * ((1 - frac) * self.buffer[kr_prev] + frac * self.buffer[kr_next]))

            # Update buffer
            self.buffer[self.kw] = x0

            self.kr = self.kr + 1 + self.W * \
                math.sin(2 * math.pi * self.f0 * n / self.rate)
